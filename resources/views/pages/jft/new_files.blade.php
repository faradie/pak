...
@section('content')
   <nav aria-label="breadcrumb">
	<ol class="breadcrumb">
		<li class="breadcrumb-item active" aria-current="page">Berkas Baru Jabatan Fungsional Tertentu</li>
	</ol>
</nav>
...
<div class="table-responsive">
	<table id="myTable" class="table table-striped">
		...
		<tbody>
			@foreach ($jft_files as $jft_file)
			<tr>
				<th  scope="row">{{$loop->iteration}}</th >
				<td>{{ strtoupper($jft_file->id) }}</td>
				<td>{{$jft_file->id_pemohon}}</td>
				<td>{{$jft_file->nama}}</td>
				<td>{{$jft_file->created_at}}</td>
				<td align="center">  
